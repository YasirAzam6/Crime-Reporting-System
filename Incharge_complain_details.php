...
<head>
    ...
    if(isset($_POST['assign']))
    {
      if($_SERVER["REQUEST_METHOD"]=="POST")
      {
        ...
        $query2 = "INSERT INTO `complaint` ('p_id') VALUES ('$pname')";
...
<!-- <select class="form-control" name="police_name" style="margin-left:40%; width:250px;">
            ...
            <select class="form-control" name="police_name" required="required" style="margin-left:40%; width:250px;">
                                      <option value="" hidden selected readonly>Choose Officer</option>
            <?php
                          $query = "SELECT * FROM `police` WHERE location='$location'";
                                                    $runQuery = mysqli_query($conn, $query);
                                                    
                                                    while ($row = mysqli_fetch_array($runQuery)) {
                                                        
                                                        $police_name = $row['p_name'];
                                                        $police_id = $row['p_id'];
                                                        ?>
                                                         <option value="<?php if (isset($police_name)) {
                                                                echo $police_name;
                                                            } ?>">
                                                        <?php if (isset($police_name)) {
                                                                echo $police_name;
                                                            } ?></option>
                                                    <?php }; ?>
...
<div style="position: fixed;
   ...
   color: white;
   text-align: center;">
  <h4 style="color: white;">&copy <b>Crime Portal <script>document.write(new Date().getFullYear())</script></b></h4>
</div>
